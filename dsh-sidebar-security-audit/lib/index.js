...
*/
const CLI_FILE_EDITORS = /* @__PURE__ */ new Set(["zed"]);
...
*/
function currentWorkspace(ctx, sessionId, cwdHint) {
	if (sessionId !== "") {
...
async function handleRequest(ctx, req, res) {
	...
	const workspace = currentWorkspace(ctx, url.searchParams.get("session") ?? "", url.searchParams.get("cwd") ?? "");
...
*/
async function zedCliCandidates() {
	const found = [];
	const add = (candidate) => {
		if (candidate !== "" && !found.includes(candidate)) found.push(candidate);
	};
	for (const name of ["zed", "zeditor"]) if ((process.env.PATH ?? "").split(path.delimiter).some((entry) => entry !== "" && fsSync.existsSync(path.join(entry, name)))) add(name);
	for (const dir of [
		path.join(os.homedir(), ".local/share/applications"),
		"/usr/share/applications",
		"/usr/local/share/applications"
	]) try {
		const exec = (await fs.readFile(path.join(dir, "dev.zed.Zed.desktop"), "utf8")).split(/\r?\n/).find((line) => line.startsWith("Exec="));
		if (exec !== void 0) {
			const command = /^Exec="([^"]+)"/.exec(exec)?.[1] ?? exec.slice(5).trim().split(/\s+/)[0] ?? "";
...
function openFileLog(message) {
	try {
		fsSync.appendFileSync(path.join(os.homedir(), ".dsh", "tmp", "zed-open.log"), `${(/* @__PURE__ */ new Date()).toISOString()} ${message}\n`);
	} catch {}
}
/**
...
async function handleOpenFile(req, res) {
	...
	if (line !== void 0 && (typeof line !== "number" || !Number.isInteger(line) || line < 1)) {
	...
	const target = line !== void 0 ? `${file}:${line}` : file;
	const candidates = await zedCliCandidates();
	for (const candidate of candidates) if (await spawnDetached(candidate, [target])) {
	...
	writeJson(res, 502, { error: candidates.length === 0 ? "no zed CLI found (tried zed, zeditor, dev.zed.Zed desktop entry)" : `failed to launch: ${candidates.join(", ")}` });
}
		...
		const dispose = ctx.webServer.register({
			...
			handler: async (req, res) => {
				try {
					...
					if ((req.method ?? "GET") === "POST" && pathname === `/api/dsh-sidebar-security-audit/open-file`) {
